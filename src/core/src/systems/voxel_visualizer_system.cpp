...
VoxelVisualizerSystem::VoxelVisualizerSystem()
{
    ...
    render_command_list_ = dx12api().CreateCommandList(render_system.current_frame_command_allocator());
...
void VoxelVisualizerSystem::InitOutput()
{
    ...
    output_ = dx12api().CreateResource(
...
void VoxelVisualizerSystem::Run(ComponentAccess& access, EntityQuery& entity_query, tf::Subflow& subflow)
{
    auto& render_system = world().GetSystem<RenderSystem>();
    ...
    Constants constants
    {
        render_system.window_width(), render_system.window_height(), render_system.frame_count(), 0,
        {voxel_system.scene_aabb().pmin.x, voxel_system.scene_aabb().pmin.y, voxel_system.scene_aabb().pmin.z},
        {voxel_system.scene_aabb().pmax.x, voxel_system.scene_aabb().pmax.y, voxel_system.scene_aabb().pmax.z}
    };
    ...
    render_command_list_->SetComputeRootDescriptorTable(RootSignature::kOutput,
                                                        render_system.GetDescriptorHandleGPU(output_descriptor_table));
    render_command_list_->SetComputeRootConstantBufferView(RootSignature::kCameraBuffer,
                                                           camera.camera_buffer->GetGPUVirtualAddress());
