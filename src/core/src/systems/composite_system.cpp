...
uint32_t CompositeSystem::PopulateDescriptorTable()
{
    ...
    dx12api().device()->CreateShaderResourceView(raytracing_system.current_frame_output(),
                                                 &srv_desc,
...
void CompositeSystem::Render(float time, uint32_t output_srv_index)
{
    ...
    command_list_->SetGraphicsRootDescriptorTable(
        RootSignature::kRaytracerOutput, render_system.GetDescriptorHandleGPU(output_srv_index));
