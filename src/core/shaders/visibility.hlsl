...
[shader("raygeneration")] void TraceVisibility() {
...
[shader("closesthit")] void Hit(inout RayPayload payload, in MyAttributes attr) {
    ...
    float3 v0 =
        float3(g_vertex_buffer[vo + 3 * i0], g_vertex_buffer[vo + 3 * i0 + 1], g_vertex_buffer[vo + 3 * i0 + 2]);
    float3 v1 =
        float3(g_vertex_buffer[vo + 3 * i1], g_vertex_buffer[vo + 3 * i1 + 1], g_vertex_buffer[vo + 3 * i1 + 2]);
    float3 v2 =
        float3(g_vertex_buffer[vo + 3 * i2], g_vertex_buffer[vo + 3 * i2 + 1], g_vertex_buffer[vo + 3 * i2 + 2]);

    float3 n0 =
        float3(g_normal_buffer[vo + 3 * i0], g_normal_buffer[vo + 3 * i0 + 1], g_normal_buffer[vo + 3 * i0 + 2]);
    float3 n1 =
        float3(g_normal_buffer[vo + 3 * i1], g_normal_buffer[vo + 3 * i1 + 1], g_normal_buffer[vo + 3 * i1 + 2]);
    float3 n2 =
        float3(g_normal_buffer[vo + 3 * i2], g_normal_buffer[vo + 3 * i2 + 1], g_normal_buffer[vo + 3 * i2 + 2]);
    ...
    t.y = 1.f - t.y;
    float3 kd = (mesh.texture_index == ~0u)
                    ? 1.f
                    : g_textures[NonUniformResourceIndex(mesh.texture_index)].SampleLevel(g_sampler, t, 0);
    kd = pow(kd, 2.2f);
    ...
    bool needs_indirect_trace = true;//((xy.x % 2) == x_offset && (xy.y % 2) == y_offset);
