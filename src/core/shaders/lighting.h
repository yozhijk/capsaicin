...
LightSample DirectionalLight_Sample(uint count)
{
    ...
    ls.intensity = 0.1f * (3.f * float3(14.f, 12.f, 10.f) + float3(0.f, 0.f, 2.f + 2.f * cos(t)));
    ls.pdf       = 1.f;
...
float3 CalculateDirectIllumination(in float3 v, in float3 n, in float3 kd, in uint shadow_hitgroup_index)
{
    ...
    return ss.intensity * kd * Lambert_Evaluate(n, ss.direction) * max(dot(n, ss.direction), 0.f);
}
